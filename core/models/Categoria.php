...
final class Categoria extends Models implements OCREND {
  ...
  final public function errores(array $data) {
    try {
     ...
     $i = array(
      'gruNombre' => $data['nombre'],
      'gruDescripcion' => $data['descripcion'],
      'gruEstado' => $data['estado']
    );
  ...
  final public function editar(array $data) : array {
    ...
    $this->db->update('grupo',$i,"gruCodigo='$this->id'",'LIMIT 1');
  ...
  final public function borrar() {
    $this->db->delete('grupo',"gruCodigo='$this->id'");
  ...
  final public function leer(bool $multi = true) {
    if($multi) {
      return $this->db->select('*','grupo');
    }

    return $this->db->select('*','grupo',"gruCodigo='$this->id'");
  }
