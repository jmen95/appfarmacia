...
final class Login extends Models implements OCREND {
  ...
  final public function SignIn(array $data) : array {
    ...
    $_SESSION['MENU'] = $this->db->select('pagname, pagurl, pagicono,nivel,peredita,perelimina','paginas inner join permisos on perpag=pagid',"pagestado='A' and FLOOR(nivel)=nivel and peridrol=".$this->u[0]['rol'],'ORDER BY nivel');
     
    $_SESSION['MENUHIJOS']=$this->db->select('pagname, pagurl, pagicono,nivel,peredita,perelimina','paginas inner join permisos on perpag=pagid',"pagestado='A' AND FLOOR(nivel)<>nivel and peridrol=".$this->u[0]['rol'],'ORDER BY nivel');
