...
class Sessions extends Models implements OCREND {
  ...
  */
  public function session_in_use(int $id) : bool {
    $time = time();
    if($this->db->rows($this->db->query("SELECT userid FROM users WHERE userid='$id' AND session >= '$time' LIMIT 1;")) > 0) {
      return true;
    }
  ...
  */
  public function check_life(bool $force = false) {
    if(isset($_SESSION[SESS_APP_ID])) {
      ...
      if($force || $this->db->rows($this->db->query("SELECT userid FROM users WHERE userid='$id' AND session <= '$time' LIMIT 1;")) > 0) {
